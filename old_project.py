...
class FaceRecognizer:
    ...
    def __call__(self):
        ...
        # Получаем фотографии и соответствующие им номера
        images, labels = self.get_images(path)
        cv2.destroyAllWindows()

        # Для распознавания используем локальные бинарные шаблоны
        recognizer = cv2.face.LBPHFaceRecognizer_create(2, 16, 8, 8, 100)
        ...
        for image_path in image_paths:
            ...
            number_predicted, conf = recognizer.predict(image)

            # Извлекаем настоящий номер фото и сравниваем с тем, что выдал алгоритм
